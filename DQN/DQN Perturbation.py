...
class DQN:
    ...
    def perturb_one_by_percent(self, state, noise_percent=0.2, seed=None):
        ...
        - Noise is uniformly sampled from [-noise_percent, +noise_percent]
        - If the selected value |x| < threshold, no perturbation is applied
        """
        ...
        # safety: do not perturb very small values
        if abs(x) < 0.1:
            noise = 0.0
            s[idx] = x
        else:
            # relative perturbation in [-noise_percent, +noise_percent]
            rel = rng.uniform(-noise_percent, noise_percent)
            noise = rel * x
            s[idx] = x + noise
    ...
    def test(self,rain):
        ...
        while not done:
            ...
            snext = self.perturb_one_by_percent(snext, noise_percent=0.2, seed=None)
            s = snext
            t += 1
